...
2. **Boundary-matched clips:** generate each clip (image-to-video) with BOTH a start-image and an end-image (pass both as reference images). Make **clip N's end-image == clip N+1's start-image** (the SAME keyframe file). When the clips play/scrub back-to-back, the handoff frame is identical → invisible seam. When extracting the frame sequence, **drop the duplicated first frame of clips 2..N**.
