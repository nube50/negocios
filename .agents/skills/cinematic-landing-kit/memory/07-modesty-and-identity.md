...
- Use `generate_image` with the product reference passed via `ImagePaths` and state in the prompt: *"keep the exact product identity / emblem / calligraphy unchanged."*
...
- For a clean nav logo: generate the emblem alone ("ONLY the emblem, flat, on pure black/white") then run `rembg` to remove the background.
